    ...
    async add(character: Character): Promise<Character | undefined> {
        try {
            const res = await client.query(
                ...
                [character.name, character.characterClass, character.items, character.attack, character.mana, character.hp, character.level]
            );
            return res.rows[0];
        } catch (error) {
    ...
    async update(id: string, character: Character): Promise<Character | undefined> {
        try {
            const res = await client.query(
                ...
                [character.name, character.characterClass, character.items, character.attack, character.mana, character.hp, character.level, id]
            );
            return res.rows[0];
        } catch (error) {
